...
export const ShiftTimingModal = (props) => {
    ...
    const [initialStartDate,setInitialStartDate]=useState(function stringToDate(){
      ...
      const [initialShiftStartTime,setInitialShiftStartTime]=useState
      ({"startHour":props.shiftstart.slice(0,2),"startMinute":props.shiftstart.slice(3,5)})
      
      const[initialShiftEndTime,setInitialShiftEndTime]=useState
      ({"endHour":props.shiftend.slice(0,2),"endMinute":props.shiftend.slice(3,5)})
...
const { vertical, horizontal, open } = state;
  ...
  const shiftTimingsModalHandle=(e)=>{
    ...
    return (
      isLoading?<Loading/>:
        <Card style={{ maxWidth: 670, padding: "10px 5px", margin: "0 auto" ,marginTop:"12px"}}>
        <CardContent>
            <center>
                <Grid>
        ...
        <Typography variant='h6'>Shift Start</Typography>
                <TextField name="startHour"  value={initialShiftStartTime.startHour} 
                onChange=
                {(e)=>{setInitialShiftStartTime({...initialShiftStartTime,startHour:e.target.value})}}
                 label="Hour" type="number"  InputProps={{ inputProps: { max:23,min:0} }} sx={{width:190}}></TextField>
        ...
        <InputLabel id="demo-multiple-name-label">Minute</InputLabel>
                ...
                // onChange={(e)=>{setInitialShiftStartTime(e.target.value)}}
        ...
        <TextField name="endHour" value={initialShiftEndTime.endHour} 
        onChange={(e)=>{setInitialShiftEndTime({...initialShiftEndTime,endHour:e.target.value})}} label="Hour"  type="number"  InputProps={{ inputProps: { max:23,min:0} }} sx={{width:190}}></TextField>
        <FormControl sx={ {width: 190,marginLeft:2.5 }}>
        <InputLabel id="demo-multiple-name-label">Minute</InputLabel>
                <Select
                name="endMinute"
            ...
            <Grid item xs={12} sx={{display:'flex',
                                ...
                                alignItems:'center'
                                  ...
                                  {/* <Grid item xs={12} sx={{justifyContent:"left",display:"flex"}}>
                     <Button onClick={handleStClose} style={{marginLeft:"32px",marginTop:"0px",color:"red",fontWeight:"bold"}}>Cancel</Button>
                    </Grid> */}
                <Button sx={{marginTop:"10px"}} type="submit" variant="contained"   style={GlobalButton.OperationButton} >UPDATE</Button>
               <Button sx={{marginLeft:"20px",marginTop:"10px"}} onClick={props.onClose3} variant='contained'  style={GlobalButton.HaltButton}>Cancel</Button>
